...
import { NAME_AUTO_SYNC_EXTRA_JSON_KEY } from '../../../lib/csvPresetUtils';
...
/**
 * Projects the `_name_auto_sync` extra_json marker for every element about to be serialized, and
 * removes a stale marker from any element that no longer needs one. Only an `_nameAutoSync ===
 * false` element whose *name* would itself be misclassified as auto-generated by
 * `wouldElementNameLookAutoGenerated` (the pattern heuristic `isNameAutoSyncEnabled` falls back to
 * when no flag is persisted) is actually at risk of a reload silently reverting it and rewriting
 * every name-based reference to it — every other explicitly named element (the common case: any
 * element named at creation carries `_nameAutoSync: false` whether or not the user ever touched
 * the name) is already safe without the marker, so this keeps the exported extra_json free of that
 * noise.
 *
 * `_nameAutoSync` flips in many places across this store (`updateElement`, `addElement`,
 * `syncAutoGeneratedElementNames`, duplication, …); projecting at this single export seam — rather
 * than writing/clearing the marker at every flip site — is what keeps a re-enabled auto-sync from
 * ever exporting a stale `false` marker: the decision here is always freshly derived from the
 * current in-memory flag and name, never carried over from a previous load. Applied once to
 * `state.elementsById` at the top of `generateCSV`, so every section's export loop
 * (`getElementsOfType`) sees the projected `extra_json` without change.
 */
const projectNameAutoSyncMarkersForExport = (state: GeometryState): Record<string, Element> => {
  const elementsById = state.elementsById;
  let changed = false;
  const next: Record<string, Element> = { ...elementsById };
  for (const id of Object.keys(elementsById)) {
    const element = elementsById[id];
    const extraJson = element.extra_json as Record<string, unknown> | undefined;
    const hasMarker = !!extraJson && Object.prototype.hasOwnProperty.call(extraJson, NAME_AUTO_SYNC_EXTRA_JSON_KEY);
    const needsMarker = element._nameAutoSync === false && state.wouldElementNameLookAutoGenerated(id);
    if (needsMarker) {
      if (!hasMarker || extraJson![NAME_AUTO_SYNC_EXTRA_JSON_KEY] !== false) {
        next[id] = {
          ...element,
          extra_json: { ...extraJson, [NAME_AUTO_SYNC_EXTRA_JSON_KEY]: false },
        } as Element;
        changed = true;
      }
    } else if (hasMarker) {
      const rest = { ...extraJson };
      delete rest[NAME_AUTO_SYNC_EXTRA_JSON_KEY];
      next[id] = {
        ...element,
        extra_json: Object.keys(rest).length > 0 ? rest : undefined,
      } as Element;
      changed = true;
    }
  }
  return changed ? next : elementsById;
};
...
export const createIoSlice = (options: IoSliceOptions): GeometryStoreSlice => {
  ...
  generateCSV: () => {
    const rawState = get();
    // Project the `_nameAutoSync` flag onto `extra_json` once, up front, so every element
    // section's export loop below (all keyed off `state.elementsById`) picks up the marker without
    // each one projecting it individually. See `projectNameAutoSyncMarkersForExport`.
    const state: GeometryState = {
      ...rawState,
      elementsById: projectNameAutoSyncMarkersForExport(rawState),
    };
    const lines: string[] = [];
  ...
  loadFromCSV: (csvContent) => {
    ...
    // This preserves manual overrides across save/load without new CSV columns.
    const zonesWithOverrideFlags = newZones.map(zone => {
      ...
      if (csvHeight > 0) {
        ...
        if (derivedH > 0 && Math.abs(csvHeight - derivedH) > 0.005) {
