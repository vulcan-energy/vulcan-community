...
/**
 * R4.6b-2 MOVE NOTE: `segmentsFromLayoutScope`, `isCanonicalArrayIndexSegment`,
 * `getAtPath`, `setAtPathNode` and `setAtPath` used to be written out here, between
 * `labelForProperty` above and `renderControlForProperty` below. They are `../lib/jsonTypes`
 * now, docstrings and logic byte-for-byte — a walk over plain JSON data has no business
 * living behind a React component module, and BOTH walks in this file (plus, once the
 * paired parent PR lands, whatever else needs a data path) reach it from there. The
 * `lodash/fp` array-hop contract and every divergence note travelled with the code.
 */
