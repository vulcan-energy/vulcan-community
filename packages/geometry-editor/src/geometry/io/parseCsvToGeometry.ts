...
export const parseCsvToGeometry = (
  ...
  for (const el of newElements) {
    ...
    // Promote the persisted manual-name marker back onto the typed field before
    // `syncAutoGeneratedElementNames` (which runs at the end of `loadFromCSV`) ever sees this
    // element: `isNameAutoSyncEnabled` short-circuits on `_nameAutoSync === false`, so this is
    // what stops the pattern heuristic from re-enabling auto-sync for a manual name that happens
    // to look auto-generated (e.g. a lone appliance named "Fridge 1"). The marker key itself is
    // left in `extra_json` — same as every other `_`-prefixed UI key on ingest — and `generateCSV`
    // re-derives it from `_nameAutoSync` on every export, so a stale value here is never load-bearing.
    if (el.extra_json && (el.extra_json as Record<string, unknown>)[NAME_AUTO_SYNC_EXTRA_JSON_KEY] === false) {
